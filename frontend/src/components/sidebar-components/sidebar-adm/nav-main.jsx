  ...
  if (isRoute) {
    return (
      <SidebarMenuButton
        ...
        tooltip={item.title}
        isActive={isActive}
        className="h-10 rounded-xl px-3 text-sidebar-foreground/95 group-data-[collapsible=icon]:size-10 group-data-[collapsible=icon]:justify-center hover:bg-white/14 data-[active=true]:bg-gray/100 data-[active=true]:text-secondary-foreground md:data-[active=true]:text-accent/50">
        <Link href={item.url}>
